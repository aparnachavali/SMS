...
def admin_home(request):
    total_staff = Instructor.objects.all().count()
    ...
    for student in students:
        attendance = AttendanceReport.objects.filter(student_id=student.id, status__in=[True]).count()
        absent = AttendanceReport.objects.filter(student_id=student.id, status__in=[False]).count()
...
def add_staff(request):
    ...
    context = {'form': form, 'page_title': 'Add Instructor'}
...
def staff_feedback_message(request):
    if request.method != 'POST':
        ...
        context = {
            'feedbacks': feedbacks,
            'page_title': 'Instructor Feedback Messages'
        }
...
def view_staff_leave(request):
    if request.method != 'POST':
        ...
        context = {
            'allLeave': allLeave,
            'page_title': 'Leave Applications From Instructor'
        }
...
def admin_notify_staff(request):
    ...
    context = {
        'page_title': "Send Notifications To Instructor",
        'allStaff': staff
    }
